...
#'
#' @param age numerical age
#' @param ps vectorized parameters in order of c(Linf, K, v, tm, t0)
#' @param setorigin constrain t_0 to 0 when TRUE
#'
#' @return numerical or vector
#' @export
...
#' }
ext_vb <- function(age, ps, setorigin = FALSE) {
  if (setorigin == FALSE) {
    x <-
      ifelse(age < ps[4],
             - ps[2] * (age - ps[5]),
             - ps[2] * ((age - ps[5]) - ps[3] * (age - ps[4]))
      )

    estim_length <- ps[1] * (1 - exp(x))
  }
